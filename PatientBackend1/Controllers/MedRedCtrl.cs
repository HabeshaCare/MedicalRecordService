...
namespace patientBackend1.Controllers
{
    ...
    public class MedRecController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> CreateMedicalRecord(MedRecDTO medRecDTO)
        {
            try
            {
                
               var  medicalRecords=  await _medRecServices.CreateMedicalRecord(medRecDTO);
                return Ok(new {data=medicalRecords,
...
// Get medical record    
    ...
    try{
        ...
        return Ok(new {data=medicalRecord,
                message= "Created Successfully"});
...
public async Task<IActionResult> UpdateMedicalRecord(MedRecDTO model, string medrecId){
    try
    {
        var medrecUpdated = await _medRecServices.UpdateMedicalRecord(model,medrecId);
        return Ok(new{data=medrecUpdated,
...
public async Task<IActionResult> AddPrescription(string medicalRecordId, Prescriptions prescription)
{
    try
    {
